...
- `PIIDetectionResponse` field 6 (`discovered_labels`) is reserved: the open-vocabulary label inbox
  was removed.
- The response carries three observability channels beyond the entities themselves:
  `discarded_entities` (what the post-filter rejected, with a reason), `detector_stats` (per-detector
  busy time and counts), and `summary` (counts per type).
